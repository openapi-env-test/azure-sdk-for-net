...
namespace Azure.ResourceManager.Hci
{
    ...
    public partial class PublisherCollection : ArmCollection, IEnumerable<PublisherResource>, IAsyncEnumerable<PublisherResource>
    {
        ...
        {
            async Task<Page<PublisherResource>> FirstPageFunc(int? pageSizeHint)
            {
                using var scope = _publisherClientDiagnostics.CreateScope("PublisherCollection.GetAll");
                scope.Start();
                try
                {
                    var response = await _publisherRestClient.ListByClusterAsync(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken).ConfigureAwait(false);
                    return Page.FromValues(response.Value.Value.Select(value => new PublisherResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
                    scope.Failed(e);
            ...
            async Task<Page<PublisherResource>> NextPageFunc(string nextLink, int? pageSizeHint)
            {
                using var scope = _publisherClientDiagnostics.CreateScope("PublisherCollection.GetAll");
                scope.Start();
                try
                {
                    var response = await _publisherRestClient.ListByClusterNextPageAsync(nextLink, Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken).ConfigureAwait(false);
                    return Page.FromValues(response.Value.Value.Select(value => new PublisherResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
        ...
        {
            Page<PublisherResource> FirstPageFunc(int? pageSizeHint)
            {
                using var scope = _publisherClientDiagnostics.CreateScope("PublisherCollection.GetAll");
                scope.Start();
                try
                {
                    var response = _publisherRestClient.ListByCluster(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken);
                    return Page.FromValues(response.Value.Value.Select(value => new PublisherResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
            ...
            Page<PublisherResource> NextPageFunc(string nextLink, int? pageSizeHint)
            {
                using var scope = _publisherClientDiagnostics.CreateScope("PublisherCollection.GetAll");
                scope.Start();
                try
                {
                    var response = _publisherRestClient.ListByClusterNextPage(nextLink, Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken);
                    return Page.FromValues(response.Value.Value.Select(value => new PublisherResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
