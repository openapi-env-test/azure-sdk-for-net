// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

namespace Azure.ResourceManager.Signall.Models
{
    /// <summary> The service properties when target service type is SelfHostedServer. </summary>
    public partial class SelfHostedServer : TargetServiceBase
    {
        /// <summary> Initializes a new instance of SelfHostedServer. </summary>
        public SelfHostedServer()
        {
            ServiceType = TargetServiceType.SelfHostedServer;
        }

        /// <summary> Initializes a new instance of SelfHostedServer. </summary>
        /// <param name="serviceType"> The target service type. </param>
        /// <param name="endpoint"> The endpoint of service. </param>
        internal SelfHostedServer(TargetServiceType serviceType, string endpoint) : base(serviceType)
        {
            Endpoint = endpoint;
            ServiceType = serviceType;
        }

        /// <summary> The endpoint of service. </summary>
        public string Endpoint { get; set; }
    }
}
